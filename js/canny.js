...
(function() {
  ...
  /**
   * apply robert filter to image data
   * @param {object} GrayImageData object
   * @return {object} GrayImageData object
   */

  CannyJS.robert = function(imgData) {
    var copy, xFiler, yFiler;
    yFiler = [[0, 1], [-1, 0]];
    xFiler = [[1, 0], [0, -1]];
    copy = imgData.copy();
    copy.fill(0);
    imgData.eachPixel(2, function(x, y, current, neighbors) {
      var ghs, gvs, i, j, _i, _j;
      ghs = 0;
      gvs = 0;
      for (i = _i = 0; _i < 2; i = ++_i) {
  ...
  /**
   * apply prewitt filter to image data
   * @param {object} GrayImageData object
   * @return {object} GrayImageData object
   */

  CannyJS.prewitt = function(imgData) {
    var copy, xFiler, yFiler;
    yFiler = [[-1, -1, -1], [0, 0, 0], [1, 1, 1]];
    xFiler = [[-1, 0, 1], [-1, 0, 1], [-1, 0, 1]];
    copy = imgData.copy();
    copy.fill(0);
    imgData.eachPixel(3, function(x, y, current, neighbors) {
      var ghs, gvs, i, j, _i, _j;
   ...
   * @param {number} [sigmma=1.4] value of sigmma of gauss function
   * @param {number} [size=3] size of the kernel (must be an odd number)
   * @param {string} sobel, robert, prewitt
   * @return {object} GrayImageData object
   */

  CannyJS.canny = function(canvas, ht, lt, sigmma, kernelSize, filter) {
    ...
    switch (filter) {
      case "sobel":
        filOpt = CannyJS.sobel(blur);
        break;
      case "robert":
        filOpt = CannyJS.robert(blur);
        break;
      case "prewitt":
        filOpt = CannyJS.prewitt(blur);
        break;
      default:
        filOpt = CannyJS.sobel(blur);
        break;
    }
    nms = CannyJS.nonMaximumSuppression(filOpt);
